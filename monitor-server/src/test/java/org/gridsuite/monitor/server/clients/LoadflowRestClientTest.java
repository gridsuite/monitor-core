...
@ContextConfiguration(classes = { LoadflowRestClient.class })
class LoadflowRestClientTest {
    ...
    void getResult() {
        server.expect(MockRestRequestMatchers.method(HttpMethod.GET))
            .andExpect(MockRestRequestMatchers.requestTo(
                "http://loadflow-server/v1/results/" + RESULT_UUID
            ))
        ...
        String result = loadflowRestClient.getResult(RESULT_UUID);
    ...
    void deleteResult() {
        server.expect(MockRestRequestMatchers.method(HttpMethod.DELETE))
            .andExpect(MockRestRequestMatchers.requestTo("http://loadflow-server/v1/results?resultsUuids=" + RESULT_UUID))
            .andRespond(MockRestResponseCreators.withSuccess());

        assertThatNoException().isThrownBy(() -> loadflowRestClient.deleteResult(RESULT_UUID));
    }
    ...
    void deleteResultFailed() {
        ...
        assertThatThrownBy(() -> loadflowRestClient.deleteResult(RESULT_UUID))
            .isInstanceOf(RestClientException.class);
    }
}
